...
const EscaperoomGame = () => {
	...
	// Sprachwechsel: ungelöstes Bücherrätsel mit den Wörtern der neuen Sprache neu mischen
	useEffect(() => {
		if (booksCorrect) return;
	...
	useEffect(() => {
		...
		if (solved && !booksCorrect) {
			sfx('booksSolved');
			setBooksCorrect(true);
		} else if (!solved && booksCorrect) {
			setBooksCorrect(false);
		}
	}, [topBooks, bottomBooks, booksCorrect, words, sfx]);
	...
	const handleSunPointerDown = (e) => {
		...
		setDraggingSun(true);
		setSunPosition(svgPoint(e.currentTarget.ownerSVGElement, e));
	};

	const handleFoodPointerDown = (e) => {
		if (catFed || !completedStations.includes(1)) return;
		if (e.cancelable) e.preventDefault();
		setDraggingFood(true);
		setFoodPosition(svgPoint(e.currentTarget.ownerSVGElement, e));
	};

	const startBookDrag = (row, fromIndex, book) => (e) => {
		if (!completedStations.includes(6) || booksCorrect) return;
		if (e.cancelable) e.preventDefault();
		e.stopPropagation();
		setDraggingBook({ row, fromIndex, letter: book.letter, color: book.color });
	...
	const handlePointerMove = (e) => {
		...
		if (draggingSun) setSunPosition(p);
		else if (draggingFood) setFoodPosition(p);
		else setDragPos(p);
	};
	...
	const handlePointerUp = (e) => {
		...
		if (draggingSun) {
			setDraggingSun(false);
			if (p.x >= 230 && p.x <= 380 && p.y >= 80 && p.y <= 280) {
				setSunInWindow(true);
				sfx('sun');
			}
		} else if (draggingFood) {
			setDraggingFood(false);
			if (p.x >= 45 && p.x <= 95 && p.y >= 640 && p.y <= 670) {
				setCatFed(true);
				sfx('cat');
			}
			...
			for (let idx = 0; idx < maxBooks; idx++) {
				const bookX = startX + idx * 32;
				if (p.x >= bookX && p.x <= bookX + 28) {
					targetIndex = idx;
					break;
				}
			}
			...
			if (targetIndex !== -1 && targetIndex !== fromIndex) {
				sfx('click');
				const setBooks = isTopRow ? setTopBooks : setBottomBooks;
				setBooks((prev) => {
					const next = [...prev];
	...
	const solvedTopBooks = makeBooks(words.top, TOP_COLORS);
	const solvedBottomBooks = makeBooks(words.bottom, BOTTOM_COLORS);
	const renderTopBooks = booksCorrect ? solvedTopBooks : topBooks;
	const renderBottomBooks = booksCorrect ? solvedBottomBooks : bottomBooks;
		...
		<div className="bg-gradient-to-br from-blue-100 to-purple-100 p-4 min-h-screen flex flex-col items-center justify-start overflow-auto">
			...
			<div className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl p-4 md:p-8 flex items-center justify-center">
				...
				>
					...
					<g>
						<rect x="230" y="80" width="150" height="200" rx="4" fill="#FFFFFF" />
					...
					<g>
						...
						{completedStations.includes(3) && !sunInWindow && !draggingSun && (
							<g
								style={{ cursor: 'grab' }}
								onMouseDown={handleSunPointerDown}
								onTouchStart={handleSunPointerDown}
							>
							...
							{!completedStations.includes(1) ? (
								...
								<>
									...
									{!catFed && !draggingFood && (
										<g
											style={{ cursor: 'grab' }}
											onMouseDown={handleFoodPointerDown}
											onTouchStart={handleFoodPointerDown}
										>
					...
					<g onClick={() => openStation(3)} style={{ cursor: canOpenStation(3) ? 'pointer' : 'default' }}>
						...
						{renderTopBooks.map((book, bookIdx) => {
							const hidden = draggingBook && draggingBook.row === 'top' && draggingBook.fromIndex === bookIdx;
							return (
								<g
									key={`top-${bookIdx}`}
						...
						{renderBottomBooks.map((book, bookIdx) => {
							const hidden = draggingBook && draggingBook.row === 'bottom' && draggingBook.fromIndex === bookIdx;
							return (
								<g
									key={`bottom-${bookIdx}`}
									style={{ cursor: booksDraggable ? 'grab' : 'default' }}
					...
					{draggingSun && (
						<g opacity="0.85" pointerEvents="none">
							<Sun x={sunPosition.x} y={sunPosition.y} r={12} />
						</g>
					)}
					{draggingFood && (
						<g pointerEvents="none">
							<FishFood x={foodPosition.x} y={foodPosition.y} opacity={0.85} />
						</g>
					)}
					{draggingBook && (
						<g pointerEvents="none" opacity="0.85">
							<rect x={dragPos.x - 14} y={dragPos.y - 32.5} width="28" height="65" rx="2" fill={draggingBook.color} />
							<text x={dragPos.x} y={dragPos.y + 2.5} textAnchor="middle" fill="white" fontSize="24" fontWeight="bold" fontFamily="Arial">
								{draggingBook.letter}
							</text>
						</g>
					)}
